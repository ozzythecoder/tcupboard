...
router.get("/profile/:auth0Id", authMiddleware, async (req, res) => {
    try {
        ...
        const query =
            "SELECT id, auth0_id, username, bio, avatar_url, tagline, created_at FROM users WHERE auth0_id = $1";
...
router.get("/", authMiddleware, async (req, res) => {
    try {
        const { rows } = await pool.query(
            "SELECT auth0_id, username, email FROM users ORDER BY username",
        );
...
router.put("/email", authMiddleware, async (req, res) => {
    ...
    console.log(
        "AUTH0_CLIENT_SECRET:",
        process.env.AUTH0_CLIENT_SECRET ? "[PRESENT]" : "[MISSING]",
    );
    ...
    try {
        ...
        if (!auth0Id) {
            console.error("No auth0Id found in request", {
                req: { auth: req.auth, user: req.user },
            });
