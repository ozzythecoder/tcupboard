    ...
    try {
        ...
        if (venvExists) {
            ...
            if (fs.existsSync(venvPythonPath)) {
                ...
                console.log(
                    `[${scraperName}] Using virtual environment Python executable: ${pythonExecutablePath}`,
                );
            } else {
                console.error(
                    `[${scraperName}] Error: Python executable not found at ${venvPythonPath}! Falling back to system Python.`,
                );
                pythonExecutablePath = process.env.PYTHON_PATH || "python3"; // Fallback
                console.log(
                    `[${scraperName}] Falling back to Python path: ${pythonExecutablePath}`,
                );
            }
            // *** END NEW LOGIC ***
        } else {
            ...
            console.log(
                `[${scraperName}] Venv not found or incomplete at ${venvPath}. Using Python path: ${pythonExecutablePath}`,
            );
        }
        ...
        try {
            console.log(
                `[${scraperName}] Spawning process: ${pythonExecutablePath} ${processArgs.join(" ")}`,
            );
            ...
            return {
                // Provide a default structure matching successful runs
                scraper_name: scraperName,
                success: false,
                errors: [`Node.js failed to spawn Python process: ${spawnError.message}`],
                added_count: 0,
                updated_count: 0,
                duplicate_count: 0,
                skipped_count: 0,
        ...
        if (fs.existsSync(logsDir)) {
            try {
                resultFiles = fs
                    ...
                    .filter(
                        (file) =>
                            file.toLowerCase().includes(scraperName.toLowerCase()) &&
                            file.endsWith("_results.json"),
                    )
                    .map((file) => ({
                        name: file,
                ...
                console.error(
                    `[${scraperName}] Error reading log directory ${logsDir}: ${fsError}`,
                );
            }
        }
        ...
        if (resultFiles.length > 0) {
            ...
            console.log(
                `[${scraperName}] Attempting to parse results from file: ${resultFiles[0].name}`,
            );
            try {
                ...
                console.error(
                    `[${scraperName}] Failed to parse results file ${resultFiles[0].name}: ${parseError}`,
                );
                result = null;
            }
        }
        ...
        if (!result && exitCode === 0) {
            ...
            try {
                ...
                console.error(
                    `[${scraperName}] Failed to parse JSON output from stdout: ${parseError}`,
                );
                result = null;
            }
        }
        ...
        if (!result) {
            console.log(
                `[${scraperName}] Creating basic result object due to lack of parsed data.`,
            );
            result = {
                scraper_name: scraperName,
                success: exitCode === 0,
                ...
                errors:
                    exitCode === 0
                        ? []
                        : [
                              `Script exited with code ${exitCode}. Stderr: ${stderrData.substring(0, 500)}${stderrData.length > 500 ? "..." : ""}`,
                          ],
        ...
        try {
            ...
            const insertResult = await db("development.scraper_logs")
                .insert({
                    scraper_name: scraperName,
                    run_at: new Date(),
            ...
            if (insertResult && insertResult.length > 0 && insertResult[0] && insertResult[0].id) {
                ...
                console.error(
                    `[${scraperName}] Failed to insert log or get returning ID. Result from insert:`,
                    insertResult,
                );
                result.errors = [
                    ...(result.errors || []),
                    "Failed to insert log into database or retrieve ID.",
                ];
            }
            ...
            if (
                logId &&
                result.added_shows &&
                result.added_shows.length > 0 /* && result.success */
            ) {
                console.log(
                    `[${scraperName}] Storing details for ${result.added_shows.length} added shows.`,
                );
                try {
                    ...
                    const showDetails = await db("development.shows")
                        .select(
                            "shows.id",
                            "shows.bands as show_name",
                            "venues.venue as venue_name",
                            "shows.start as show_date",
                        )
                    ...
                    if (showDetails.length > 0) {
                        ...
                        console.warn(
                            `[${scraperName}] Could not find details in 'shows' table for added show IDs: ${result.added_shows.join(", ")}`,
                        );
                    }
                } catch (dbError) {
                    console.error(
                        `[${scraperName}] Error retrieving or storing added show details: ${dbError}`,
                    );
                    result.errors = [
                        ...(result.errors || []),
                        `Error storing added show details: ${dbError.message}`,
                    ];
                    if (result.error_count !== undefined) result.error_count++;
                    else result.error_count = 1;
                }
            }
        } catch (dbError) {
            console.error(
                `[${scraperName}] FATAL: Error logging scraper run to database: ${dbError}`,
            );
            result.success = false;
            result.errors = [
                ...(result.errors || []),
                `Error logging run to database: ${dbError.message}`,
            ];
            if (result.error_count !== undefined) result.error_count++;
            else result.error_count = 1;
...
router.get("/scraper-history", authMiddleware, checkRole(["admin"]), async (req, res) => {
    try {
        ...
        const logs = await db("development.scraper_logs")
            ...
            .leftJoin(
                "development.scraper_show_additions",
                "scraper_logs.id",
                "scraper_show_additions.scraper_log_id",
            )
        ...
        const [{ count }] = await db("development.scraper_logs").count("* as count");
...
router.get("/scraper-logs/:id", authMiddleware, checkRole(["admin"]), async (req, res) => {
    try {
        ...
        // Get the log
        const log = await db("development.scraper_logs").where("id", id).first();
