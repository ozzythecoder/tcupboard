        ...
        if (existingConv) {
            ...
            const { error: updateError } = await supabase
                ...
                .update({
                    ...
                    unread_count_user1:
                        user1 === userId2
                            ? existingConv.unread_count_user1 + 1
                            : existingConv.unread_count_user1,
                    unread_count_user2:
                        user2 === userId2
                            ? existingConv.unread_count_user2 + 1
                            : existingConv.unread_count_user2,
                })
                .match({ user1, user2 });
    ...
    try {
        ...
        const conversationsWithProfiles = conversations.map((conv) => {
            ...
            const unreadCountForThisUser =
                conv.user1 === auth0_id ? conv.unread_count_user1 : conv.unread_count_user2;
...
router.get("/conversation/:userId", authMiddleware, async (req, res) => {
    try {
        ...
        const { data: messages, error } = await supabase
            ...
            .select("*")
            .or(
                `and(sender_id.eq.${auth0_id},recipient_id.eq.${otherUserId}),and(sender_id.eq.${otherUserId},recipient_id.eq.${auth0_id})`,
            )
        ...
        const accessibleMessages = messages.filter((msg) => checkMessageAccess(auth0_id, msg));
...
router.get("/conversation-by-id/:conversationId", authMiddleware, async (req, res) => {
    try {
        ...
        console.log(
            `Fetching conversation ${conversationId} for user ${auth0_id} (page ${page}, limit ${limit})`,
        );
        ...
        const otherUserId =
            conversation.user1 === auth0_id ? conversation.user2 : conversation.user1;
        ...
        console.log(
            `Pagination: Total ${totalMessages}, Page ${page}/${totalPages}, Range ${from}-${to}`,
        );
...
router.post("/send", authMiddleware, async (req, res) => {
    try {
        ...
        const conversationId = await updateOrCreateConversation(
            auth0_id,
            recipient_id,
            content,
            now,
...
router.post("/:conversationId/read", authMiddleware, async (req, res) => {
    try {
        ...
        const updateData =
            conversation.user1 === auth0_id ? { unread_count_user1: 0 } : { unread_count_user2: 0 };
...
router.put("/:messageId/read", authMiddleware, async (req, res) => {
    try {
        ...
        if (message.recipient_id !== auth0_id) {
            return res
                .status(403)
                .json({ error: "You can only mark messages sent to you as read" });
        }
