...
router.get("/debug", async (req, res) => {
    try {
        // Hard-coded auth0Id for testing
        const auth0Id = "auth0|676f46146e408cedfac8db04";

        const query = `
      SELECT b.*, ba.role
      FROM ${schema}.bands_new b
      JOIN ${schema}.band_admins ba ON b.id = ba.band_id
      WHERE ba.auth0_id = $1
...
router.get("/user", authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const auth0Id = req.user?.sub || req.auth?.sub;

        if (!auth0Id) {
            console.error("No user ID found in request:", { user: req.user, auth: req.auth });
            return res.status(401).json({ error: "User ID not found in token" });
        }

        console.log("Fetching bands for user:", auth0Id);

        // Make sure schema is defined
        const dbSchema = schema || "public"; // Fallback to public schema if undefined

        // Get all bands where user is an admin/owner AND include drafts
        const query = `
      SELECT b.*, ba.role
      FROM ${dbSchema}.bands_new b
      JOIN ${dbSchema}.band_admins ba
      ON b.id = ba.band_id
      WHERE ba.auth0_id = $1
      ORDER BY b.updated_at DESC
    `;

        console.log("Executing query with schema:", dbSchema);

        const result = await pool.query(query, [auth0Id]);
        console.log(`Found ${result.rows.length} bands for user`);
    ...
    try {
      ...
      `,
          ...
          `,
            ...
            if (formData.instruments) {
                await client.query(
                    `DELETE FROM ${schema}.band_member_instruments WHERE member_id IN 
          (SELECT id FROM ${schema}.band_members WHERE band_id = $1)`,
                    [bandId],
                );

                // Add new instruments for each member
                const members = await client.query(
                    `SELECT id, name FROM ${schema}.band_members WHERE band_id = $1`,
                    [bandId],
                );
            ...
            // Update genres
            if (formData.genres) {
                await client.query(`DELETE FROM ${schema}.band_genres WHERE band_id = $1`, [
                    bandId,
                ]);
            ...
            if (formData.releases) {
                await client.query(`DELETE FROM ${schema}.band_releases WHERE band_id = $1`, [
                    bandId,
                ]);
      ...
      `,
            ...
            if (hasMerch && formData.merchTypes) {
                await client.query(`DELETE FROM ${schema}.band_merch_types WHERE band_id = $1`, [
                    bandId,
                ]);
      ...
      `,
            ...
            if (lookingForMembers && formData.openPositions) {
                await client.query(`DELETE FROM ${schema}.band_open_positions WHERE band_id = $1`, [
                    bandId,
                ]);
            ...
            if (formData.groupSizes) {
                await client.query(`DELETE FROM ${schema}.band_group_sizes WHERE band_id = $1`, [
                    bandId,
                ]);
        ...
        `,
            ...
            const { customSlug, profileTheme, headerLayout, backgroundPattern, backgroundImage } =
                formData;
      ...
      `,
            ...
            if (formData.featuredContent) {
                await client.query(
                    `DELETE FROM ${schema}.band_featured_content WHERE band_id = $1`,
                    [bandId],
                );
          ...
          `,
            ...
            if (formData.profileBadges) {
                await client.query(`DELETE FROM ${schema}.band_profile_badges WHERE band_id = $1`, [
                    bandId,
                ]);
...
router.get("/draft/:id", authMiddleware, async (req, res) => {
    ...
    `,
        ...
        // Get band data
        const bandResult = await pool.query(`SELECT * FROM ${schema}.bands_new WHERE id = $1`, [
            bandId,
        ]);
       ...
       FROM ${schema}.band_featured_content WHERE band_id = $1 ORDER BY display_order`,
        ...
        try {
            if (bandResult.rows[0].completion_status) {
                completionStatus =
                    typeof bandResult.rows[0].completion_status === "string"
        ...
        try {
            if (bandResult.rows[0].music_links) {
                musicLinks =
                    typeof bandResult.rows[0].music_links === "string"
            ...
            if (bandResult.rows[0].social_links) {
                socialLinks =
                    typeof bandResult.rows[0].social_links === "string"
...
router.put("/:slug/edit", authMiddleware, async (req, res) => {
    ...
    `,
        ...
        await client.query(`DELETE FROM ${schema}.band_featured_content WHERE band_id = $1`, [
            bandId,
        ]);
        await client.query(`DELETE FROM ${schema}.band_profile_badges WHERE band_id = $1`, [
            bandId,
        ...
        if (req.body.updated_images) {
            for (const image of req.body.updated_images) {
                if (image.deleted) {
                    await client.query(`DELETE FROM ${schema}.band_images WHERE id = $1`, [
                        image.id,
                    ]);
