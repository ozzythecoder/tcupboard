...
import { ForumMessageWithReplyDetails } from "#/types/resources";
import type { Pagination as IPagination } from "#/types/apiResponse";
import { postsService, usePostsService } from "#/services/posts";
import { ApiError } from "#/lib/api";

interface Props {
    posts: ForumMessageWithReplyDetails[];
    pagination: IPagination;
    onPageChange: (event: React.ChangeEvent<unknown, Element>, newPage: number) => void;
}

const ListOfAllThreads = ({ posts, pagination, onPageChange }: Props) => {
    ...
    useEffect(() => {
        const fetchReadStatus = async () => {
            try {
                setLoading(true);
                const threadsToCheck = posts.flatMap((v) => (v.id ? [v.id.toString()] : []));
                const response = await postsService.readStatus(threadsToCheck)
    ...
    const isThreadUnread = (post) => {
        ...
        const latestActivity =
            lastReplyTimestamp && lastReplyTimestamp > threadTimestamp
                ? lastReplyTimestamp
                : threadTimestamp;
    ...
    const formatDate = (rawDate) => {
        ...
        if (Number.isNaN(postDate.valueOf())) {
            return rawDate;
        }
                ...
                <List sx={{ p: 0 }}>
                    {posts.map((post) => {
                        ...
                        const lastReplyDate = post.last_reply_at
                            ...
                            : "";
                        const { author, author_avatar } = post;
                                            ...
                                            <Box sx={{ width: "100%", pl: 6 }}>
                                                ...
                                                >
                                                    ...
                                                    {post.tags && post.tags.length > 0 && (
                                                        <Box>
                                                            <ActiveTags
                                                                tags={post.tags.slice(0, TAG_LIMIT)}
                                                                small
                                                            />
                                                        </Box>
                                                    )}
