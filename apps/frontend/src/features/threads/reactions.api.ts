    ...
    toggleReaction: (api: ProtectedApi) =>
        mutationOptions({
            ...
            onMutate: async (vars, ctx) => {
                // optimistic update
                console.debug("post reaction: beginning optimistic udpate");
                ...
                ctx.client.setQueryData(key, (old: PostReactionWithUsername[]) => {
                    ...
                    if (existingReaction) {
                        console.debug("Existing reaction found: removing");
                        return old.filter((e) => e.userId !== Number(userId));
                    }
                    console.debug("No existing reaction found: adding");
