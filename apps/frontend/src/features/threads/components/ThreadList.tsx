...
export function ThreadListItem({ thread }: { thread: ForumMessageWithReplyDetails }) {
    ...
    return (
        <li className="card p-4 grid grid-cols-[auto_1fr] preset-filled-surface-50-950 has-[.-thread-title-anchor:hover]:preset-filled-primary-50-950">
            <div className="flex flex-col items-center gap-4 w-25">
                ...
                <Link to="/" from="/threads/">
                    <p className="anchor text-sm text-surface-400-600">{thread.author}</p>
                </Link>
            </div>
            <Link to="/threads/$id" params={{ id: thread.id!.toString() }} className="-thread-title-anchor group">
                <div className="flex flex-col justify-between gap-4 h-full">
                    <h3 className="h6 group-hover:underline">{thread.title}</h3>
                    <p className="text-sm text-surface-400-600">
